...
directory_path = "data/prob_extracted_mean/MPI/ssp370/yearly/r1"
...
directory_path = "data/prob_extracted_var/MPI/ssp370/yearly"
files = readdir(directory_path, join = true)
...
power_r3mpi_location_over_time_lower = []
power_r3mpi_location_over_time_upper = []
for year in years
    #year = 2015
    print(year)
    filtered_df = CSV.read(
    ...
    r3_wind_speeds_gp_turbines_variance =
        sqrt.(data_frames_variances["r1_wind_speeds_turbines"*string(year)])
    print("pre-process")
    r3_wind_speeds_gp_turbines_lower =
        r3_wind_speeds_gp_turbines .- 1 .* r3_wind_speeds_gp_turbines_variance
    r3_wind_speeds_gp_turbines_upper =
        r3_wind_speeds_gp_turbines .+ 1 .* r3_wind_speeds_gp_turbines_variance
    r3_wind_speeds_gp_turbines_lower =
        [collect(row) for row in eachrow(r3_wind_speeds_gp_turbines_lower)]
    r3_wind_speeds_gp_turbines_upper =
        [collect(row) for row in eachrow(r3_wind_speeds_gp_turbines_upper)]
    print("start height-pred")
    r3_wind_speeds_gp_turbines_lower = map(
        (x, y) -> wind_speed_to_hub_height(x, y),
        r3_wind_speeds_gp_turbines_lower,
        filtered_df.hub_height,
    )
    r3_wind_speeds_gp_turbines_upper = map(
        (x, y) -> wind_speed_to_hub_height(x, y),
        r3_wind_speeds_gp_turbines_upper,
        filtered_df.hub_height,
    )
    print("start power pred")
    @suppress begin
        global power_r3mpi_location_lower = map(
    ...
    @suppress begin
        global power_r3mpi_location_upper = map(
            py"compute_windpower",
            r3_wind_speeds_gp_turbines_upper,
            filtered_df.turbine_name,
            #repeat(["E-53/800"], length(r3_wind_speeds_gp_turbines)),
    ...
    push!(
        power_r3mpi_location_over_time_upper,
        mean(power_r3mpi_location_upper) * nrow(filtered_df) ./ 1e4,
    )
    push!(
        power_r3mpi_location_over_time_lower,
        mean(power_r3mpi_location_lower) * nrow(filtered_df) ./ 1e4,
    )
end

#save timeseries as csv
power_r3mpi_location_over_time_all_upper = []
for i = 1:length(power_r3mpi_location_over_time_upper)
    push!(
        power_r3mpi_location_over_time_all_upper,
        power_r3mpi_location_over_time_upper[i],
    )
end
power_r3mpi_location_over_time_all_upper = reshape(
    collect(Iterators.flatten(power_r3mpi_location_over_time_all_upper)),
    :,
)
df = DataFrame(vector = power_r3mpi_location_over_time_all_upper)
CSV.write(
    "data/power_forecasts/historical/0012_historical_ssp370_pos_uncertainty_1std.csv",
    df,
)

power_r3mpi_location_over_time_all_lower = []
for i = 1:length(power_r3mpi_location_over_time_lower)
    push!(
        power_r3mpi_location_over_time_all_lower,
        power_r3mpi_location_over_time_lower[i],
    )
end
power_r3mpi_location_over_time_all_lower = reshape(
    collect(Iterators.flatten(power_r3mpi_location_over_time_all_lower)),
    :,
)
df = DataFrame(vector = power_r3mpi_location_over_time_all_lower)
CSV.write(
