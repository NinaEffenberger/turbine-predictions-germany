...
x = randn(11)
f = GP(Matern52Kernel())
cov(f(x, S)) == kernelmatrix(Matern52Kernel(), x) + S
cov(f(x, S))

s = rand(11);
Diagonal(s)
cov(f(x, s)) == kernelmatrix(Matern52Kernel(), x) + Diagonal(s)
...
for i in years
    ...
    for j in collect(1:2)
        u = data_u["uas"][:, :, indices_per_year[1]-1+j]
        v = data_v["vas"][:, :, indices_per_year[1]-1+j]
        u2 = data_u["uas"][:, :, indices_per_year[1]-1+j] .+ 0.1
        v2 = data_v["vas"][:, :, indices_per_year[1]-1+j] .+ 0.1
        u = collect(Iterators.flatten(transpose(u)))
        v = collect(Iterators.flatten(transpose(v)))
        ...
        gp = GP(kron_kernel)
        # compute variance
        A = [((u[i] - u2[i])^2 / 2) for i = 1:length(u)]
        B = [((v[i] - v2[i])^2 / 2) for i = 1:length(v)]
        S = vcat(A, B)
        fx = gp(X, Diagonal(S))
        p_fx = posterior(fx, Y)
        ...
        vars = mean_and_var(p_fx, MOInput(new_X, 2))
        var = vars[2]
        speeds = reshape(speeds, Int(length(speeds) / 2), 2)
        var = reshape(var, Int(length(var) / 2), 2)
        u = speeds[:, 1]
        v = speeds[:, 2]
        u_var = var[:, 1]
        v_var = var[:, 2]
        speeds_timepoint = vec(sqrt.(u .^ 2 .+ v .^ 2))
        vars_timepoint = vec(sqrt.(u_var .^ 2 .+ v_var .^ 2))
        print(vars_timepoint)
    ...
    CSV.write(
        "data/extracted_wind_speeds/MPI/" *
        pathway *
        "/yearly/ens_wind_speeds_turbines" *
        string(i) *
        ".csv",
        df,
    )
