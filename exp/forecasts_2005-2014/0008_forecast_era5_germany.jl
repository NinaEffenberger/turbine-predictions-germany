...
for j in collect(1:length(nc["time"][:]))
    ...
    #strength_loc = map((x, y) -> py"compute_windpower"(x, y), speeds_timepoint, filtered_df.turbine_name)
...
loc_mean_speeds
loc_mean_speeds[length(loc_mean_speeds)-1460:length(loc_mean_speeds)]
...
df = DataFrame(orig_mean_speeds, :auto)
CSV.write("data/extracted_wind_speeds/ERA5/2014/orig_wind_speeds.csv", df)
...
factor1 = cumsum(power_mean_time_mw)[365] / cumsum(true_generation)[365]
factor2 = cumsum(location_time_mw)[365] / cumsum(true_generation)[365]

factors1 = Vector{Float64}()
factors2 = Vector{Float64}()
for i in collect(1:365)
...
ax = Axis(f[1, 1])
lines!(collect(1:365), factors1 ./ maximum(factors1), color = "red")
#lines!(collect(1:365), abs.(cumsum(orig_low_res_vector)/factor-cumsum(forecasted_gernation[1:365]), color = "blue"))
lines!(collect(1:365), factors2 ./ maximum(factors2), color = "green")
...
ax = Axis(f[1, 1])
lines!(
    collect(1:length(power_mean_time_mw)),
    (
        cumsum(power_mean_time_mw[1:length(power_mean_time_mw)]) ./
        mean(factors1) .- cumsum(true_generation[1:length(power_mean_time_mw)])
    ),
    color = "red",
)
lines!(
    collect(1:length(power_mean_time_mw)),
    (
        cumsum(location_time_mw[1:length(power_mean_time_mw)]) ./
        mean(factors2) .- cumsum(true_generation[1:length(power_mean_time_mw)])
    ),
    color = "green",
)
