...
true_gen = lines!(
    ...
    diff(cumsum(true_generation[1:365*4:end])),
    color = "black",
)
ssp126t = lines!(
    ax,
    1:length(ssp126_turbines[1:365*4:end])-1,
    diff(cumsum(ssp126_turbines[1:365*4:end]) / factor_ssp126_turbines),
)
ssp126 = lines!(
    ax,
    1:length(ssp126_turbines[1:365*4:end])-1,
    diff(cumsum(ssp126_orig[1:365*4:end]) / factor_ssp126_orig),
    linestyle = :dot,
)
ssp245t = lines!(
    ax,
    1:length(ssp245_turbines[1:365*4:end])-1,
    diff(cumsum(ssp245_turbines[1:365*4:end]) / factor_ssp245_turbines),
)
ssp245 = lines!(
    ax,
    1:length(ssp245_turbines[1:365*4:end])-1,
    diff(cumsum(ssp245_orig[1:365*4:end]) / factor_ssp245_orig),
    linestyle = :dot,
)
ssp370t = lines!(
    ax,
    1:length(ssp370_turbines[1:365*4:end])-1,
    diff(cumsum(ssp370_turbines[1:365*4:end]) / factor_ssp370_turbines),
)
ssp370 = lines!(
    ax,
    1:length(ssp370_turbines[1:365*4:end])-1,
    diff(cumsum(ssp370_orig[1:365*4:end]) / factor_ssp370_orig),
    linestyle = :dot,
)
ssp585t = lines!(
    ax,
    1:length(ssp585_turbines[1:365*4:end])-1,
    diff(cumsum(ssp585_turbines[1:365*4:end]) / factor_ssp585_turbines),
)
ssp585 = lines!(
    ax,
    1:length(ssp585_turbines[1:365*4:end])-1,
    diff(cumsum(ssp585_orig[1:365*4:end]) / factor_ssp585_orig),
    linestyle = :dot,
)
Legend(
    f[1, 2],
...
save("plots/paper/pdfs/power_generation_cmip6_diff.pdf", f)
f
