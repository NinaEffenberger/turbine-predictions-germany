...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp126_turbines_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp126_turbines_future, cumsum(df.value)[end])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp245_turbines_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp245_turbines_future, cumsum(df.value)[end])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp370_turbines_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp370_turbines_future, cumsum(df.value)[end])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp585_turbines_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp585_turbines_future, cumsum(df.value)[end])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp126_orig_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp126_orig_future, cumsum(df.vector)[1])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp245_orig_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp245_orig_future, cumsum(df.vector)[1])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp370_orig_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp370_orig_future, cumsum(df.vector)[1])
    data_frames[base_name] = df
end
...
files = readdir(directory_path, join = true)
data_frames = Dict{String,DataFrame}()
ssp585_orig_future = Float64[]
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    push!(ssp585_orig_future, cumsum(df.vector)[1])
    data_frames[base_name] = df
end
...
with_theme(T) do
    ...
    ax = Axis(
        ...
        xticks = (1:5:5*6, ["2025", "2030", "2035", "2040", "2045", "2050"]),
        #xlabelsize = 15,
        #ylabelsize = 15,
        #xticklabelsize = 12,
        #yticklabelsize = 12,
    ...
    ssp126t = lines!(
        ...
        (ssp126_turbines_future) / factor_ssp126_turbines,
        color = "#0072B2",
    )
    #ssp126 = lines!(
    #    ax,
    #    1:length(ssp126_orig_future),
    #   (ssp126_orig_future) / factor_ssp126_orig,
    #   linestyle = :dot,
    #   color = "#0072B2",
    #)
    ssp245t = lines!(
        ...
        (ssp245_turbines_future) / factor_ssp245_turbines,
        color = "#E69F00",
    )
    #ssp245 = lines!(
    #    ax,
    #    1:length(ssp245_orig_future),
    #    (ssp245_orig_future) / factor_ssp245_orig,
    #    linestyle = :dot,
    #    color = "#E69F00",
    #)
    ssp370t = lines!(
        ...
        (ssp370_turbines_future) / factor_ssp370_turbines,
        color = "#009E73",
    )
    #ssp370 = lines!(
    #    ax,
    #    1:length(ssp370_orig_future),
    #    (ssp370_orig_future) / factor_ssp370_orig,
    #    linestyle = :dot,
    #    color = "#009E73",
    #)
    ssp585t = lines!(
        ...
        (ssp585_turbines_future) / factor_ssp585_turbines,
        color = "#CC79A7",
    )
    #ssp585 = barplot!(
    #    ax,
    #    1:length(ssp585_orig_future),
    #    (ssp585_orig_future) / factor_ssp585_orig,
    #    linestyle = :dot,
    #    color = "#CC79A7",
    #)
    #true_gen = lines!(
    ...
    save("plots/pdfs/0008_power_generation_cmip6_normalize2023_yearly.pdf", f)
    f
end
...
# Example data
timesteps = [string(i) for i = 1:26]
makes = ["SSP126", "SSP245", "SSP370", "SSP585"]

# Random values for each make at each timestep
values = [
    ssp126_turbines_future ./ factor_ssp126_turbines ./ 10^6,  # Make A values across timesteps
    ssp245_turbines_future ./ factor_ssp245_turbines ./ 10^6,  # Make B values across timesteps
    ssp370_turbines_future ./ factor_ssp370_turbines ./ 10^6,
    ssp585_turbines_future ./ factor_ssp585_turbines ./ 10^6,  # Make C values across timesteps
]

ssp245_turbines_future ./ factor_ssp245_turbines  # Make B values across timesteps
ssp370_turbines_future ./ factor_ssp370_turbines
# Number of timesteps and makes
n_timesteps = length(timesteps)
n_makes = length(makes)

# X-axis positions for each group of bars
bar_width = 0.2
x_positions = [
    (1:n_timesteps) .- bar_width * (n_makes / 2) .+ bar_width * i for
    i = 0:n_makes-1
]
...
with_theme(T) do
    ...
    ax = Axis(
        fig[1, 1],
        ylabel = "Power generation (TWh)",
        #xticks = (1:n_timesteps, timesteps),
        xticks = (1:5:5*6, ["2025", "2030", "2035", "2040", "2045", "2050"]),
    )

    # Plotting each make's bars at each timestep
    for (i, make) in enumerate(makes)
        barplot!(ax, x_positions[i], values[i], label = make, width = bar_width)
    end
    hlines!(ax, 118.78, color = :black, linewidth = 2)
    ...
    # Display the plot
    save("plots/pdfs/0009_barplot_yearly_powergen.pdf", fig)
    fig
end
