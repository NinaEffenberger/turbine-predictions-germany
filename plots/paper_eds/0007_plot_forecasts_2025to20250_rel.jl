...
for file in files
    base_name = splitext(basename(file))[1]
    df = CSV.read(file, DataFrame)
    n = Int(min(ncol(df), 1460))
    normalized = df.value[1:n] ./ factor_ssp126_turbines ./ true_generation_2023
    append!(
        ssp126_turbines_future,
        DataFrame(value = normalized),
        cols = :union,
    )
    data_frames[base_name] = df
end
...
for file in files
    ...
    df = CSV.read(file, DataFrame)
    n = Int(min(ncol(df), 1460))
    normalized = df.value[1:n] ./ factor_ssp245_turbines ./ true_generation_2023
    append!(
        ssp245_turbines_future,
        DataFrame(value = normalized),
        cols = :union,
    )
    data_frames[base_name] = df
...
for file in files
    ...
    df = CSV.read(file, DataFrame)
    n = Int(min(ncol(df), 1460))
    normalized = df.value[1:n] ./ factor_ssp370_turbines ./ true_generation_2023
    append!(
        ssp370_turbines_future,
        DataFrame(value = normalized),
        cols = :union,
    )
    data_frames[base_name] = df
...
for file in files
    ...
    df = CSV.read(file, DataFrame)
    n = Int(min(ncol(df), 1460))
    normalized = df.value[1:n] ./ factor_ssp585_turbines ./ true_generation_2023
    append!(
        ssp585_turbines_future,
        DataFrame(value = normalized),
        cols = :union,
    )
    data_frames[base_name] = df
